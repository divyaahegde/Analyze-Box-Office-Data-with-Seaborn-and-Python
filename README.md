...
## 8. Analyzing Movie Release Dates

If you look at the release_date column, you will realize that we need to convert it into a standardized format for us to analyze it. We also need to specify if it was a 20th or 21st century film. 

### Preprocessing Features
If the value of the year is <= 19, we can prepend 20 in front of the year else prepend a 19. Use this function:
```python

```

Check if there's any movie with a NaN value as the release_date. If it is just a handful of movies, we can append the dates with help of google.

`check re`

We find out that one movie has a NaN value as its release date. Look it up and change it as shown below:

`cgsw`

## 9. Feature Engineering Release Date

We will convert the above column generated into a standard date-time format in python so that it is easy to extract features like month, year, etc.

`jjj`

After this, create a function to preprocess the datetime format and create new columns for year, week day, etc.




## Visualize number of Films released per year



You can start off by creating two variables for train and test datasets.
`kjkj`
